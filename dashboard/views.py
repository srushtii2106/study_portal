...
def notes(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            notes.save()
        messages.success(request, f"Notes Added from {request.user.username} Successfully" )  # Use f-string to format the message
    else:
        form = NotesForm()    
...
def homework(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            homeworks.save()
            messages.success(request, f"Homework Added from {request.user.username} !!" )# Use f-string to format the message
    else:
        form = HomeworkForm()
...
def youtube(request):
    if request.method == "POST":
        ...
        for i in video.result()['result']: # type: ignore
            result_dict = {
                'input':text,
                'title':i['title'], # type: ignore
                'duration':i['duration'], # type: ignore
                'thumbnail':i['thumbnails'][0]['url'], # type: ignore
                'channel':i['channel']['name'], # type: ignore
                'link':i['link'], # type: ignore
                'views':i['viewCount']['short'], # type: ignore
                'published':i['publishedTime'] # type: ignore
            }
            desc=''
            if i['descriptionSnippet']: # type: ignore
                for j in i['descriptionSnippet']: # type: ignore
                    desc += j['text'] # type: ignore
...
def delete_todo(requet,pk=None):
...
def register(request):
    ...
    return render(request,"../templates/dashboard/register.html",context)
